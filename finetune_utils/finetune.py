...
def train(model_path : str, data_dir : str, output_dir : str, training_cfg : Dict, device: str = "cuda", use_lora: bool = False, lora_cfg: Dict = None):
    ...
    training_args = TrainingArguments(
        ...
        num_train_epochs=int(training_cfg.get('num_train_epochs', 20)),
        ...
        save_total_limit=int(training_cfg.get('save_total_limit', 100)),
...
if __name__ == "__main__":
    ...
    parser.add_argument("--training_cfg", type=str, default="finetune_utils/training_config.yaml",
                        help="Path to the training configuration file")
    parser.add_argument("--lora_cfg", type=str, default="finetune_utils/lora_config.yaml",
    ...
    if args.training_cfg:
        import yaml
        if os.path.exists(args.training_cfg):
            with open(args.training_cfg, 'r') as f:
                training_cfg = yaml.safe_load(f)
            print(f"Successfully loaded training configuration from {args.training_cfg}: {training_cfg}")
        else:
            print(f"Warning: Configuration file {args.training_cfg} does not exist, using default parameters.")
    
    lora_cfg = {}
    if args.lora and args.lora_cfg:
        import yaml
        if os.path.exists(args.lora_cfg):
            with open(args.lora_cfg, 'r') as f:
                lora_cfg = yaml.safe_load(f)
            print(f"Successfully loaded LoRA configuration from {args.lora_cfg}: {lora_cfg}")
        else:
            print(f"Warning: LoRA configuration file {args.lora_cfg} does not exist, using default LoRA parameters.")
    ...
    train(args.model_path, args.data_dir, args.output_dir, training_cfg, device="cuda" if torch.cuda.is_available() else "cpu", use_lora=args.lora, lora_cfg=lora_cfg)